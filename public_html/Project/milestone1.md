...
<tr><td> <em>Generated: </em> 4/2/2022 6:53:32 PM</td></tr>
...
<tr><td> <em>Sub-Task 4: </em> Explain briefly how the process/code works</td></tr>
<tr><td> <em>Response:</em> <p>Upon form submission (onsubmit), the validate javascript function is called.  In this<br>function, a check of the email/username field is made.  In short, the<br>program runs a regex comparison for the entered information.  If it has<br>an &#39;@&#39; symbol, but still does not follow conventional email format, then a<br>flash message is shown saying &quot;Invalid email address.&quot;  If the field contains<br>information that does not have an &#39;@&#39; symbol, then the check for valid<br>username is made.  The same process is used in this section of<br>the code--the only differenc is the regex.  We compare the textfield to<br>see if it contains letters, numbers, underscores, or dashes, and if it is<br>between 3 to 16 characters.  As for a password validation, the entered<br>information is checked to be at least 8 characters in length.  If<br>not, a flash message is shown saying &quot;Password is too short.&quot;  If<br>either condition is failed, the return value of the validation function is false.<br> Otherwise, it returns true.</p><br><p>For the PHP code, there are a set of<br>checks made that are very similar in structure to the set of changes<br>in registration.php.  Fields are initially checked to be empty.  If they<br>are filled, the email/username field is checked.  If it contains an &#39;@&#39;<br>symbol, then the email check is performed using is_valid_email.  Otherwise, a username<br>check is performed using is_valid_username.  A password check is then made, initially<br>checking if the field is empty.  If it is populated, is_valid_password is<br>used to validate the length of the password.  If all these checks<br>are successful, the program moves on to the query phase.</p><br><p>For the code screenshot,<br>a query is prepared and executed in the try statement.  In short,<br>the entered username is first checked if it exists in the database, and<br>if so, the password is checked.  If the password matches (through password_verify,<br>which compares hashes), the roles of the user are fetched.  Once all<br>this goes through successfully, the user and user roles are saved to the<br>session and the appropriate navbar and welcome message is shown.<br></p><br></td></tr>
</table></td></tr>
